...
image::https://github.com/ProgrammingRobotsStudyGroup/Designs-And-Notes/blob/master/assembly/images/a_star_bridge_pinout_dia.jpg[A-Star Pin Out Diagram]
 ...
 * This library includes an example for testing the PID constants which can be used as an alternative to the A-Star sample. It uses pin numbers for the DFRobot Romeo, however, so those may need changing. Open via File > Examples > SimplePID > RomeoPIDTest. *NEED AN A-STAR EXAMPLE*.
